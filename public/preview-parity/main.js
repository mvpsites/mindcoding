...
function boot() {
  ...
  // ---- I. Amphora: two-handled Greek vessel profile ----
  function amphora() {
    // profile: [radius, y] from foot up to lip
    const P = [
      [0.02, -1.55], [0.28,-1.5],[0.34,-1.4],[0.18,-1.28],[0.16,-1.15],
      [0.42,-0.95],[0.72,-0.55],[0.92,-0.05],[1.0,0.35],[0.95,0.72],
      [0.78,1.0],[0.55,1.22],[0.42,1.36],[0.5,1.45],[0.56,1.5],[0.5,1.55]
    ];
    const body = lathe(P, 46000, 0.014);
    // two handles: torus-ish arcs from shoulder to neck
    const hcount = 8000;
    const hpos = new Float32Array(hcount*3);
    for (let i=0;i<hcount;i++){
      const side = i < hcount/2 ? 1 : -1;
      const u = Math.random();                    // along handle
      const ang = Math.PI * (0.15 + u*0.9);       // arc
      const hx = Math.cos(ang)*0.42 + 0.78;
      const hy = Math.sin(ang)*0.62 + 0.55;
      const tube = 0.055;
      const ta = Math.random()*Math.PI*2, tr = Math.sqrt(Math.random())*tube;
      hpos[i*3]   = side * (hx + Math.cos(ta)*tr);
      hpos[i*3+1] = hy - 0.35 + Math.sin(ta)*tr;
      hpos[i*3+2] = Math.sin(ta)*tr;
    }
    return merge([body, {pos:hpos,n:hcount}]);
  }
  ...
  // =========================================================================
  const exhibits = [
    { key:'amphora', gen: amphora, hue: 0.09, tint:[0.95,0.90,0.80] },
